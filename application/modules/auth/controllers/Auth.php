...
//			$this->template->main( $this->data);
	...
	public function edit_user( $id ) {
		...
		foreach ( $currentGroups as $data ) {
			array_push( $currentGroups, $data->id );
		}
			...
			if ( $this->ion_auth->is_admin() ) {
				...
				if ( isset( $groupData ) && ! empty( $groupData ) ) {
					...
					foreach ( $groupData as $key => $grp ) {
						...
						$this->ion_auth->add_to_group( $key, $id );
					}
				}
			}

			// check to see if we are updating the user
			if ( $this->ion_auth_model->update_user( $user->id, $data ) ) {
				// redirect them back to the admin page if admin, or to the base url if non admin
				$this->session->set_flashdata( 'notice', 'Berhasil memperbaharui data pengguna' );
				if ( $this->ion_auth->is_admin() ) {
	...
	public function api_username() {
		...
		if ( isset( $old_username ) && $this->ion_auth->user()->row()->username == $old_username ) {
